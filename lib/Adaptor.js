...
function addRow(db, table, rowData) {
  ...
  return function (state) {
    ...
    var body = "SUBJECT=LetUsDoOpenFn";
    ...
    var url = 'https://reportsapi.zoho.com/api/'.concat(account, '/', db, '/', table, '?ZOHO_ACTION=ADDROW&ZOHO_OUTPUT_FORMAT=JSON\n                  &ZOHO_ERROR_FORMAT=JSON&authtoken=', authToken, '&ZOHO_API_VERSION=', apiVersion);
    ...
    return (0, _Client.post)({ url: url, body: body }).then(function (result) {
